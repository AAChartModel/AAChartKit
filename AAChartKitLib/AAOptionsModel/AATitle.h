...
AAPropStatementAndFuncStatement(strong, AATitle, NSNumber *, y);//标题相对于垂直对齐的偏移量，取值范围：图表的上边距（chart.spacingTop ）到图表的下边距（chart.spacingBottom），可以是负值，单位是px。默认值和字体大小有关。
AAPropStatementAndFuncStatement(assign, AATitle, BOOL          , useHTML);//是否 使用HTML渲染标题。 默认是：false.
