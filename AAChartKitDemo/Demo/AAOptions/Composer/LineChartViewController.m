...
- (id)chartConfigurationWithSelectedIndex:(NSInteger)selectedIndex {
    switch (selectedIndex) {
        case 0: return [LineChartViewController customconnectNullsWithZonesForLineChart];//自定义 connectNulls + zones 实现同一个 line chart 局部断开和局部重连
        case 1: return [LineChartViewController fixedWidthSlotsLineChartWithOnePointAndRightExtension];//固定宽度槽位: 1 个点 + 右侧补线
        case 2: return [LineChartViewController fixedWidthSlotsLineChartWithTwoPoints];//固定宽度槽位: 2 个点
        case 3: return [LineChartViewController fixedWidthSlotsLineChartWithThreePoints];//固定宽度槽位: 3 个点
        case 4: return [LineChartViewController fixedWidthSlotsLineChartWithTenPoints];//固定宽度槽位: 10 个点
