...
- (id)chartConfigurationWithSelectedIndex:(NSUInteger)selectedIndex {
    switch (selectedIndex) {
        case 0: return [JSFunctionForAAChartEventsComposer setCrosshairAndTooltipToTheDefaultPositionAfterLoadingChart]; //图表加载完成后,自动设置默认的十字准星和浮动提示框的位置
        ...
        case 3: return [JSFunctionForAAChartEventsComposer configureBlinkMarkerChart]; //配置闪烁的标记点
        case 4: return [JSFunctionForAAChartEventsComposer configureSpecialStyleMarkerOfSingleDataElementChartWithBlinkEffect]; //配置单个数据元素的特殊样式标记点即闪烁特效
        case 5: return [JSFunctionForAAChartEventsComposer configureScatterChartWithBlinkEffect]; //配置散点图的闪烁特效
        case 6: return [JSFunctionForAAChartEventsComposer automaticallyHideTooltipAfterItIsShown]; //图表加载完成后,自动隐藏浮动提示框
